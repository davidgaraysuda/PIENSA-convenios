...
import { findInvoiceByIdAndCustomer, listInvoiceByCustomerId, createInvoice, updateInvoice } from '../../Services/invoiceService'
...
function InvoicePage({ customer }) {
  ...
  useEffect(() => {
    ...
    if (!!customer.id)
      listInvoiceByCustomerId(customer.id).then(data => {
        setInvoices(data);
        setLoading(false)
      }
      );
